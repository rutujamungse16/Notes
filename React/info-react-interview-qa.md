...
### Infosys Interview Prep — 4 Years Experience (React Frontend + Spring Boot Backend)

> 100 Questions Total: 50 Conceptual + 50 Coding Questions

## About the Infosys Interview Process (React Developer, Experienced)

Based on recent candidate reports, the typical Infosys process for a React/full-stack role at this experience level looks like:

- **Round 1 — Technical Screening**: Usually virtual, 30–45 minutes. Covers React fundamentals, JavaScript core concepts (closures, `this`, async/await, ES6+), hooks, state management approach, and a light live-coding or whiteboard-style problem. For full-stack profiles, expect Spring Boot/Java basics and REST API integration questions mixed in.
- **Round 2 — Technical Deep Dive / Architecture**: For 4+ YOE candidates, this round leans into *why* over *what* — performance optimization decisions, component reusability, scaling a large codebase, handling enterprise concerns (security, deployment pipelines, CI/CD awareness), and real project walkthroughs from your resume.
- **Round 3 — HR / Managerial Round**: Non-technical — background, project ownership, teamwork, notice period, and compensation discussion.

**What Infosys interviewers tend to emphasize for experienced candidates:**
1. Strong fundamentals explained simply (they often ask you to explain concepts as if teaching a junior dev).
2. Practical, project-grounded answers — be ready to map every concept below to a real example from your own projects.
3. Performance and scalability reasoning (code splitting, memoization, virtualization, efficient API design).
4. Full-stack integration fluency — since your stack is React + Spring Boot, expect specific questions on CORS, JWT auth flow, and REST contract design (Sections G and O below are your highest-yield prep material).
5. Clear communication — Infosys interview feedback frequently notes clarity and structured answers as much as technical correctness.

**Tip:** Read through Sections A, C, D, and G first — these map most closely to what's reported as commonly asked. Then use the coding sections (H–P) to make sure you can *write* what you can *explain*.
...
## Quick Reference: Suggested Prep Strategy for Infosys Rounds

1. **Days 1–2 (Round 1 prep — Fundamentals)**: Sections A, B, and N (Core concepts, Hooks, JS fundamentals). Infosys screening rounds lean heavily on "explain it simply" style questions — practice answering Q1–Q22 out loud in under 60 seconds each.
2. **Days 3–4 (Round 1 prep — Light coding)**: Sections H and J (Custom hooks, list/array manipulation) plus Section N (debounce, deep clone, flatten array, event emitter, bind polyfill) — these are classic quick live-coding asks.
3. **Days 5–6 (Round 2 prep — Architecture & performance)**: Sections C, D, and K (Performance, State Management, Performance Coding) — be ready to justify trade-offs (Context vs Redux, when to reach for `useMemo`, virtualization vs pagination) since Round 2 is scenario- and reasoning-driven, not definition recall.
4. **Days 7–8 (Round 2 prep — Full-stack integration)**: Sections G and O (React + Spring Boot integration, both conceptual and coding) — this is your differentiator as a full-stack candidate; walk through your own project's auth flow (JWT), CORS setup, and error-handling contract between React and Spring Boot.
5. **Day 9**: Sections I, L, M, P (Design patterns, Redux Toolkit, async/API handling, misc) — round these out with mock explanations tied to a real project you've built.
6. **Day 10**: Mock interview — pick 10 random questions across all sections and answer without looking, then check yourself against the answers here.

**Infosys-specific tips:**
- Structure every answer as: **definition → why it matters → a concrete example from your project**. Infosys feedback often flags "correct but unstructured" answers.
- For the HR round, be ready to discuss ownership of specific modules/features, how you handled a production issue, and your reason for the move (career growth, tech stack, etc.) — keep it concise and positive.
- If asked about your current CTC/notice period, have a clear, consistent number ready; Infosys recruiters often follow up on this early.
- Since the role is React + Spring Boot, expect at least one question that explicitly asks you to trace a request end-to-end (React component → Axios call → Spring Boot controller → service → repository → response → UI update). Practice narrating that flow fluently using Q47–Q50 and Q94–Q97 as your script.
